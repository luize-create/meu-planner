    ...
    return (
      <div key={h.id} style={{ display: "flex", alignItems: "stretch", borderBottom: "1px solid #0d0d18" }}>
        <div style={{ width: COL, flexShrink: 0, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button onClick={() => deletar(h.id)} style={{ background: "none", border: "none", color: "#2e2e4e", cursor: "pointer", fontSize: 10, padding: 0 }}>✕</button>
        </div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, padding: "5px 0" }}>
          <div style={{ display: "flex" }}>
            {dias.map(d => {
              ...
              if (!futuro && feito) {
                ...
                bg = "#dc2626"; shadow = "0 0 4px #dc262640"; content = "✕"
              }
              return (
                <div key={d} onClick={() => toggleDia(h.id, d)} title={`${d}/${mes + 1}`} style={{ width: CELL, height: 18, borderRadius: 4, background: bg, border: ehHoje ? "1.5px solid #7c3aed60" : "1px solid transparent", cursor: futuro ? "default" : "pointer", opacity: futuro ? 0.35 : 1, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, color: "#fff", fontWeight: 600, boxShadow: shadow, transition: "all .15s", flexShrink: 0 }}>
                  {content}
                </div>
              )
            })}
          ...
          <div style={{ display: "flex" }}>
            {dias.map(d => {
              ...
              return (
                <div key={d} onClick={() => toggleDia(h.id, d)} style={{ width: CELL, height: 8, borderRadius: 3, background: futuro ? "transparent" : bg, border: ehHoje ? "1px solid #7c3aed30" : "none", cursor: futuro ? "default" : "pointer", opacity: futuro ? 0.3 : 1, flexShrink: 0, transition: "all .15s" }} />
              )
            })}
          </div>
        </div>
    ...
    <div style={{ display: "flex", gap: 20, padding: "24px 28px", color: "#e2e8f0" }}>
        ...
        <div style={{ background: "#0f0f1c", border: "1px solid #1a1a2e", borderRadius: 16, overflow: "hidden" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #1a1a2e" }}>
            ...
            <div style={{ display: "flex", gap: 10, fontSize: 10, color: "#6b6b8a", flexWrap: "wrap" }}>
              {[{ cor: "#7c3aed", label: "Completo" }, { cor: "#1e1e2e", label: "Não feito" }, { label: "Dia perfeito", star: true }, { cor: "#059669", label: "Dia limpo" }, { cor: "#dc2626", label: "Recaída" }].map((l: any, i) => (
                ...
                {dias.map(d => {
                  ...
                  return (
                    <div key={d} style={{ width: CELL, flexShrink: 0, textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 9, fontWeight: ehHoje ? 700 : 400, color: ehHoje ? "#a855f7" : perfeito ? "#f59e0b" : "#4a4a6a" }}>{perfeito ? "⭐" : d}</div>
                    </div>
                  )
                })}
              </div>
        ...
        <div style={{ background: "#0f0f1c", border: "1px solid #1a1a2e", borderRadius: 14, padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, justifyContent: "center" }}>
              {[{ cor: "#7c3aed", label: "Concluídos", val: totalFeito }, { cor: "#1e1e2e", label: "Não concluídos", val: totalPossivel - totalFeito }, { cor: "#f59e0b", label: "Dias perfeitos", val: diasPerfeitos }].map((item, i) => (
      ...
      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "#00000092", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, backdropFilter: "blur(8px)" }}>
          <div style={{ background: "#0d0d18", border: "1px solid #1a1a2e", borderRadius: 20, padding: 24, width: 480, maxWidth: "92vw", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              {[{ v: "positivo", label: "Hábito positivo", prefix: "✓", cor: "#7c3aed", sub: "Marcar quando fizer" }, { v: "negativo", label: "Hábito negativo", prefix: "✕", cor: "#dc2626", sub: "Marcar quando recair" }].map(t => (
                <button key={t.v} onClick={() => setNovo({ ...novo, tipo: t.v as any, icone: t.v === "negativo" ? "🚬" : "🧠" })} style={{ padding: "14px 12px", borderRadius: 12, border: "2px solid", borderColor: novo.tipo === t.v ? t.cor : "#1e1e35", background: novo.tipo === t.v ? t.cor + "15" : "#12121f", cursor: "pointer", textAlign: "center", boxShadow: novo.tipo === t.v ? `0 0 20px ${t.cor}25` : "none", transition: "all .2s" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: "#4a4a6a", marginBottom: 10, fontWeight: 500 }}>Ícone</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6 }}>
                {iconesList.map(({ ic }) => {
                  const sel = novo.icone === ic
                  const cor = novo.tipo === "negativo" ? "#dc2626" : "#7c3aed"
                  return (
                    <button key={ic} onClick={() => setNovo({ ...novo, icone: ic })} style={{ width: "100%", aspectRatio: "1", borderRadius: 10, border: "1px solid", borderColor: sel ? cor : "#1e1e35", background: sel ? cor + "18" : "#12121f", cursor: "pointer", fontSize: 22, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: sel ? `0 0 12px ${cor}40` : "none", transform: sel ? "scale(1.08)" : "scale(1)", transition: "all .15s" }}>{ic}</button>
                  )
                })}
              </div>
            </div>
            ...
            <button onClick={adicionarHabito} disabled={!novo.nome.trim()} style={{ width: "100%", background: !novo.nome.trim() ? "#1e1e35" : novo.tipo === "negativo" ? "linear-gradient(135deg, #dc2626, #ef4444)" : "linear-gradient(135deg, #7c3aed, #a855f7)", border: "none", borderRadius: 10, padding: "13px", color: novo.nome.trim() ? "#fff" : "#4a4a6a", cursor: novo.nome.trim() ? "pointer" : "not-allowed", fontSize: 14, fontWeight: 600, transition: "all .2s" }}>
